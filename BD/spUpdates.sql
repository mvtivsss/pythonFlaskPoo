...
CREATE OR REPLACE PROCEDURE TURISMO.SPUPDATECHECKOUT(ID    TURISMO.RESERVA.ID_RESERVA % TYPE,
                                                     TOTAL TURISMO.RESERVA.TOTAL_RESERVA % TYPE)
  AS
  id_aux NUMBER;
  BEGIN
    UPDATE RESERVA
      SET ESTADO_RESERVA = '0', TOTAL_RESERVA = TOTAL
      WHERE ID_RESERVA = ID;
    COMMIT;
    SELECT TURISMO.orden.NEXTVAL INTO ID_AUX FROM DUAL "d";
    INSERT INTO ORDENPAGO (ID, TOTAL_PAGO, ESTADO, FECHA_REGISTRO, ID_RESERVA) VALUES (ID_AUX, TOTAL, 'inpago', SYSDATE ,ID);
    COMMIT;
  EXCEPTION
...
CREATE OR REPLACE PROCEDURE TURISMO.spUpdateOrderPay (
  id_aux TURISMO.ORDENPAGO.ID%TYPE
)
AS
BEGIN
  UPDATE ORDENPAGO SET ESTADO = 'Pagado' WHERE ID = ID_AUX;

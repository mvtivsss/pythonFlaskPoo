...
CREATE OR REPLACE PROCEDURE TURISMO.spAddUser (
 ...
 BEGIN
  ...
  INSERT INTO USUARIO (ID_USU, NOM_USU, APP_USU, APM_USU, FNACIMIENTO_USU, EMAIL_USU, CELULAR_USU,CONTRASENA_USU, COMUNA_ID_COMUNA, TIPO_USUARIO_ID_TIPO)
  VALUES (ID, NOM,APP,APM,TO_DATE(FNACIMIENTO,'DD/MM/YYYY'),EMAIL,CELULAR,CONTRASENA,IDCOMUNA,TIPOUSUARIO);
  COMMIT;
...
CREATE OR REPLACE PROCEDURE TURISMO.SPADDMULTA(CANTIDAD  IN NUMBER,
  ...
  AS
    ...
    CANT   NUMBER;
  BEGIN
    SELECT SEQ_INGRESOMULTA.nextval
      INTO ID_AUX
      FROM DUAL;
    SELECT "a".VALOR_MULTA * CANTIDAD
      INTO CANT
      FROM ACTA "a"
      WHERE "a".ID_ACTA = ACTAID;

    INSERT INTO RESERVA_ACTA (
      ID, SUBTOTAL_ACTA, ACTA_ID_ACTA, RESERVA_ID_RESERVA
    )
    VALUES (ID_AUX, CANT, ACTAID, RESERVAID);
  END;
/